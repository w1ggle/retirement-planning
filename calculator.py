...
incomeTaxesToPay = effectiveTaxRate = capitalGainsTaxesToPay = 0
...
for index in range(1,len(FED_TAX_INCOME_BUCKETS)): # go through each tax bracket
    ...
    incomeTaxesToPay +=  ((FED_TAX_INCOME_BUCKETS[index] - FED_TAX_INCOME_BUCKETS[index - 1])* (FED_TAX[index] / 100)) #else add the tax from this bucket and move to the next one

    
    

effectiveTaxRate = (incomeTaxesToPay / W2_INCOME) * 100
print("You have a marginal tax rate of {:.2f}% and have to pay ${:,.2f} in taxes".format(effectiveTaxRate, incomeTaxesToPay))   
